function WorldLandmark = InverseMeasurementModel(pose, z, intrin, CamOffset)
    c_psi = cos(pose(6));
    s_psi = sin(pose(6));
    c_theta = cos(pose(5));
    s_theta = sin(pose(5));
    ...
    WorldLandmark = R' * TempLandmark + EB_R*CamOffset; % rot.transpose() corresponds to EB_R * BC_R